...
After `ea-kit init`, install an adapter safely with `ea-kit link-host <codex|claude|cursor|gemini|antigravity>`. Use `--dry-run` to preview and `--force` only when replacing an existing host rule file.
...
| Gemini / Antigravity | Copy `gemini/GEMINI.md` to the project root as `GEMINI.md`; it delegates to the portable core. |
